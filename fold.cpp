...
class Object {
 public:
  ...
  void print() { printImpl(std::make_index_sequence<std::tuple_size_v<decltype(members)>>{}); }

  std::string toString() {
    return toStringImpl(std::make_index_sequence<std::tuple_size_v<decltype(members)>>{});
  }
  std::tuple<Ts...> members;

 protected:
  template <size_t... Is>
  void printImpl(std::index_sequence<Is...>) {
    std::initializer_list<int>{// comma fold. The value of the empty pack is void ()
  ...
  template <size_t... Is>
  std::string toStringImpl(std::index_sequence<Is...>) {
    std::stringstream ss;
    bool noComma = true;
    std::initializer_list<bool>{
        (ss << (noComma ? "" : ", ") << std::get<Is>(members), noComma = false)...};
    return ss.str();
  }
};
...
auto min(const T &t, const Ts &... ts) {
  using CommonType = typename std::common_type_t<T, Ts...>;
  auto result = static_cast<CommonType>(t);
  std::initializer_list<int>{(
      ((CommonType)ts < (CommonType)result ? result = static_cast<CommonType>(ts) : result), 0)...};
  return result;
}

template<size_t...Is>
auto Generate(std::index_sequence<Is...>) {
    std::string item{"/dev/cpu/"};
    return std::list<std::string>{item+std::to_string(Is)...};
}
...
//  std::cout << std::setprecision(6) << min(35, 34UL, (short)33, 32.3, (char)32.3)
//            << std::endl;  //  should  be 32
    std::list<std::string_view> l1{"222","22","2"};
        for (auto const &v:l1) {
            std::cout<<v<<std::endl;
        }
    auto ret =Generate(std::make_index_sequence<3>{});
    std::initializer_list<std::string> il={"222","22","2"};
    //std::list<std::string> l{Generate(std::make_index_sequence<3>{})};
    //std::list<std::string> l{ret};
