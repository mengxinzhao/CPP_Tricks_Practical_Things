...
template <typename L1, typename L2> struct S : L1, L2 {
  S(L1 l1, L2 l2) : L1(std::move(l1)), L2(std::move(l2)) {}
  using L1::operator();
  using L2::operator();
};

template <typename L1, typename L2> auto make_combined(L1 &&l1, L2 &&l2) {
...
int main() {
  ...
  for_each(data.cbegin(), data.cend(),
           [&v2](const auto &elem) { std::visit(v2, elem); });
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (i % 3 == 1)
      data2[i] = 0.5f;
    if (i % 3 == 2)
      data2[i] = std::to_string(i);
  }
  sum_int = 0;
  sum_float = 0;
  sum_common = 0;

  for_each(data2.cbegin(), data2.cend(),
           [&v2](const auto &elem) { std::visit(v2, elem); });
  std::cout << sum_int << std::endl;
  std::cout << sum_float << std::endl;
  std::cout << sum_common << std::endl;
